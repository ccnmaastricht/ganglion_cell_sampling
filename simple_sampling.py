...
class RetinalCompression:
    ...
    def create_mapping(self, parameters):
        ...
        for i in range(num_pixels_out):
            ...
            x = np.minimum(np.maximum([floor(new_y[i]), ceil(new_y[i])], 0), self.inS - 1)
            y = np.minimum(np.maximum([floor(new_x[i]), ceil(new_x[i])], 0), self.inS - 1)
    ...
    def distort_image(self, image):
        try:
            [height, width, depth] = image.shape  # Determine dimensions of the selected image (pixel space)
        except:
            [height, width] = image.shape  # Determine dimensions of the selected image (pixel space)
            height = None
